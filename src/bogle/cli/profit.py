...
def _render(report: ProfitReport, period: str, console: Console) -> None:
    ...
    console.print(f"  Ganho de capital:      {_signed(report.capital_total)}")
    console.print(f"    Realizado (vendas):  {_signed(report.realized)}")
    console.print(f"    Nao realizado:       {_signed(report.unrealized)}")
    console.print()
    income_window = " (ultimos 12 meses)" if period == "12m" else ""
    console.print(f"  Proventos recebidos:{income_window}   {_signed(report.income_total)}")
    for kind, label in _INCOME_LABELS:
        console.print(f"    {label:<19}{_signed(report.income_by_type[kind])}")
    console.print()
    if period == "12m":
        console.print("  (Lucro total omitido: ganho de capital e desde o inicio; proventos, 12 meses.)")
    else:
        console.print(f"  Lucro total:           {_signed(report.total)}")
    if report.unpriced:
        console.print(
